export default {
  nav: {
    logo: 'DSB',
    links: [
      { text: 'Projetos', to: 'mywork' },
      { text: 'Contato', to: 'mycontact' },
    ],
  },
  header: {
    img:
      '',
    imgPlaceholder:
      'data:image/jpeg;base64,',
    text: ['Olá!', "Me chamo Daniel.", ''],
    typical: [
      'desenvolvedor full stack jr. 🖥',
      2000,
      'apaixonado por games e tecnologia. 💚',
      2000,
    ],
    btnText: 'Ver mais',
  },
  work: {
    title: 'Projetos',
    img: [
      process.env.PUBLIC_URL + '/assets/tanamesa.png',
      process.env.PUBLIC_URL + '/assets/trivia.png',
      process.env.PUBLIC_URL + '/assets/cookmaster.png'
    ],
    imgPlaceholder:
      'data:image/png;base64,',
    projectName: ['Tá na Mesa!', 'Trivia', 'Cookmaster API'],
    desc: [
      'O aplicativo "Tá Na Mesa" foi idealizado para auxiliar as pessoas a terem a receita certa na palma da mão. A partir do consumo de um conjunto de APIs, qualquer pessoa pode ser um mestre cuca, com várias receitas de bebidas ou comidas acessadas pelo aplicativo.',
      'Um jogo de perguntas e respostas baseado no Trivia, com sistema de pontuação, perfil do usuário (Gravatar) e ranking, utilizando React e Redux.',
      'API que permite a postagem, visualização, atualização e remoção de receitas, com sistema de registro de usuários e autenticação com JWT. Foi utilizado NodeJS, Express e MongoDB.'
    ]},
  stack: {
    title: 'Stack',
    tech: [
      {
        img: process.env.PUBLIC_URL + '/assets/java-script.png',
        alt: 'javascript',
      },
      {
        img: process.env.PUBLIC_URL + '/logo512.png',
        alt: 'react',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/redux.png',
        alt: 'redux',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/mongo.png',
        alt: 'mongodb',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/mysql.png',
        alt: 'mysql',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/node.png',
        alt: 'node',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/express.png',
        alt: 'express',
      },
    ],
    desc: ``,
  },
  contact: {
    title: 'Entre em contato!',
    desc:
      "Será um prazer te conhecer e compartilhar conhecimento 😎",
    socials: [
      {
        alt: 'github',
        img: process.env.PUBLIC_URL + '/assets/github.png',
        link: 'https://github.com/DSborini',
      },
      {
        alt: 'link',
        img: process.env.PUBLIC_URL + '/assets/link.png',
        link: 'https://www.linkedin.com/in/danielsborini/',
      },
      {
        alt: 'gmail',
        img: process.env.PUBLIC_URL + '/assets/gmail.png',
        link: 'mailto:dsouzaborini@gmail.com',
      },
    ],
    img:
      'https://cdn.discordapp.com/attachments/794680692705067009/877316811190833213/undefined_-_Imgur.gif',
    imgPlaceholder:
      'data:image/jpeg;base64,',
  },
};
